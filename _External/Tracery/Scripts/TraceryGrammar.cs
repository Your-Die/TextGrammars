...
namespace UnityTracery
{
    ...
    public class TraceryGrammar
    {
        ...
        public TraceryGrammar(JSONObject source)
        ...
        {
            this.RawGrammar = (source ?? "").Trim();
            if (!this.ParseRulesJson())
                throw new Exception("Input grammar is not valid JSON.");

            // Set up the standard modifiers.
            this.ModifierLookup = new Dictionary<string, Func<string, string>>
            {
                {"a", Modifiers.Article},
        ...
        {
            var result = this.Parse("#origin#", randomSeed);
            return result;
        }
        ...
        public string Parse(string input, int? randomSeed = null)
        {
            ...
            var output = this.ParseInner(input);
        ...
        private static void AddCharToToken(char ch, ref string output, GrammarToken token, bool escaped = false)
        {
            ...
            {
                var openToken = token.FindLowestOpenToken();
                openToken.AddChar(ch, escaped);
            }
        }
        ...
        {
            return modifiers
                .Where(modName => this.ModifierLookup.ContainsKey(modName))
                .Select(modName => this.ModifierLookup[modName])
                .Aggregate(baseText, (cumulative, mod) => mod(cumulative));
        }
        ...
        internal string ResolveSymbol(string symbol)
        {
            ...
            if (this.SaveData[symbol])
            {
                return this.SaveData[symbol][this.SaveData[symbol].Count - 1].Random().str;
            }

            if (this.Grammar[symbol] != null)
            {
                if (this.Grammar[symbol].IsArray)
                    return this.Grammar[symbol].Random().str;

                if (this.Grammar[symbol].IsString)
                    return this.Grammar[symbol].str;
            }
        ...
        internal void PushAction(string key, params string[] options)
        {
            ...
            {
                this.SaveData[key] = JSONObject.arr;
            }

            this.SaveData[key].Add(values);
        }
        ...
        internal void PopAction(string key)
        {
            ...
            this.SaveData[key].RemoveAt(this.SaveData[key].Count - 1);
            if (this.SaveData[key].Count == 0)
            {
                this.SaveData.RemoveField(key);
            }
        }

        private bool ParseRulesJson()
        {
            this.Grammar = new JSONObject(this.RawGrammar);
            if (!this.Grammar.IsObject)
            {
                this.Grammar = null;
                return false;
            }
